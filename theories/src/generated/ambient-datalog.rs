...
relation pamb_proj_c3_b1_p0(Proc, Name, Proc, mettail_runtime :: HashBag < Proc > , Name, Proc, Proc);

pamb_proj_c3_b1_p0(parent.clone(), n.clone(), p.clone(), rest.clone(), m.clone(), r.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
    if let Proc :: PAmb(elem_f0, elem_f1) = elem,
...
rw_proc(parent, result) <--
    pamb_proj_c3_b1_p0(parent, cap_n_p0, cap_p_p0, cap_rest_p0, cap_m_p0, cap_r_p0, elem_0),
