...
pamb_proj_c3_b0_p0(parent.clone(), rest.clone(), p.clone(), n.clone(), m.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
...
bag }, let rest = elem_f1_inner_f0_rest.clone(), let p = elem_f1_inner_f0_elem_0_f1_val.clone(), let n = elem_f0_val.clone(), let m = elem_f1_inner_f0_elem_0_f0_val.clone();
...
rw_proc(parent, result) <--
    pamb_proj_c3_b0_p0(parent, cap_rest_p0, cap_p_p0, cap_n_p0, cap_m_p0, elem_0),
