...
relation pamb_proj_c3_b0_p0(Proc, Proc, Name, mettail_runtime :: HashBag < Proc > , Name, Proc);

pamb_proj_c3_b0_p0(parent.clone(), p.clone(), n.clone(), rest.clone(), m.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
...
bag }, let p = elem_f1_inner_f0_elem_0_f1_val.clone(), let n = elem_f0_val.clone(), let rest = elem_f1_inner_f0_rest.clone(), let m = elem_f1_inner_f0_elem_0_f0_val.clone();
...
relation pamb_proj_c3_b1_p0(Proc, Proc, mettail_runtime :: HashBag < Proc > , Name, Name, Proc, Proc);

pamb_proj_c3_b1_p0(parent.clone(), p.clone(), rest.clone(), m.clone(), n.clone(), r.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
    if let Proc :: PAmb(elem_f0, elem_f1) = elem,
...
rw_proc(parent, result) <--
    pamb_proj_c3_b0_p0(parent, cap_p_p0, cap_n_p0, cap_rest_p0, cap_m_p0, elem_0),
...
rw_proc(parent, result) <--
    pamb_proj_c3_b1_p0(parent, cap_p_p0, cap_rest_p0, cap_m_p0, cap_n_p0, cap_r_p0, elem_0),
