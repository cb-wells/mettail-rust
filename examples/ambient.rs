...
fn main() {
    ...
    if rewrites.len() > 1 {
        println!("NOTE: We have more than 1 rewrite! PNew congruence may be working!");
    }
    ...
    if direct_rewrites.is_empty() && !is_nf {
        println!("\n❌ ERROR: No rewrites found and not marked as normal form!");
        println!("This suggests the PNew congruence rule is not firing.");
    } else if !direct_rewrites.is_empty() && is_nf {
        println!("\n❌ ERROR: Has rewrites but marked as normal form!");
    } else if direct_rewrites.is_empty() && is_nf {
        println!("\n⚠️  Marked as normal form (no rewrites available)");
        println!("This was the BUG - PNew congruence not firing!");
    } else {
        println!("\n✓ FIXED! Can rewrite (not a normal form) - PNew congruence works!");
    }
...
pamb_proj_c3_b0_p0(parent.clone(), m.clone(), n.clone(), p.clone(), rest.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
...
bag }, let m = elem_f1_inner_f0_elem_0_f0_val.clone(), let n = elem_f0_val.clone(), let p = elem_f1_inner_f0_elem_0_f1_val.clone(), let rest = elem_f1_inner_f0_rest.clone();
...
pamb_proj_c3_b0_p1(parent.clone(), cap_m.clone(), cap_r.clone(), elem.clone()) <--
    ...
    let cap_m = (* * f0).clone(),
    let cap_r = (* * f1).clone();
relation pamb_proj_c3_b1_p0(Proc, mettail_runtime :: HashBag < Proc > , Proc, Proc, Name, Name, Proc);
pamb_proj_c3_b1_p0(parent.clone(), rest.clone(), r.clone(), p.clone(), n.clone(), m.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
    if let Proc :: PAmb(elem_f0, elem_f1) = elem,
...
bag }, for (elem_f1_inner_f0_elem_1, _count_elem_f1_inner_f0_1) in elem_f1_inner_f0.iter(), if & elem_f1_inner_f0_elem_1 != & elem_f1_inner_f0_elem_0, let rest = elem_f1_inner_f0_elem_0_f1_inner_f0_rest.clone(), let r = elem_f1_inner_f0_elem_1.clone(), let p = elem_f1_inner_f0_elem_0_f1_inner_f0_elem_0_f1_val.clone(), let n = elem_f1_inner_f0_elem_0_f0_val.clone(), let m = elem_f1_inner_f0_elem_0_f1_inner_f0_elem_0_f0_val.clone();
...
rw_proc(parent, result) <--
    pamb_proj_c3_b0_p0(parent, cap_m, cap_n_p0, cap_p_p0, cap_rest_p0, elem_0),
...
bag_result });
rw_proc(parent, result) <--
    pamb_proj_c3_b1_p0(parent, cap_rest_p0, cap_r_p0, cap_p_p0, cap_n_p0, cap_m_p0, elem_0),
...
rw_proc(parent, result) <--
    ...
    let result = Proc :: PNew(scope_tmp);
rw_proc(s, t) <--
    proc(s),
