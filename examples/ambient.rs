...
fn main() {
    ...
    // let rdx_str = "{n[{open(a,p),a[{in(b,q)}]}], b[{new(x,r)}]}";
    let rdx_str = "new(x,new(y,p))";
...
bag }, let rest = p0_rest.clone(), let p = body_0.clone(), let x = binder_0.clone(), if is_fresh(& x, & p), let p1 = (Proc :: PNew(mettail_runtime :: Scope :: from_parts_unsafe(x.clone(), Box :: new(Proc :: PPar({ let mut bag = mettail_runtime :: HashBag :: new();
bag.insert(p.clone());
...
pamb_proj_c3_b0_p1(parent.clone(), cap_m.clone(), cap_r.clone(), elem.clone()) <--
    ...
    let cap_m = (* * f0).clone(),
    let cap_r = (* * f1).clone();
relation pamb_proj_c3_b1_p0(Proc, Proc, mettail_runtime :: HashBag < Proc > , Name, Proc, Name, Proc);
pamb_proj_c3_b1_p0(parent.clone(), p.clone(), rest.clone(), m.clone(), r.clone(), n.clone(), elem.clone()) <--
    proc(parent),
    if let Proc :: PPar(ref bag_field) = parent,
    for (elem, _count) in bag_field.iter(),
    if let Proc :: PAmb(elem_f0, elem_f1) = elem,
...
rw_proc(parent, result) <--
    pamb_proj_c3_b1_p0(parent, cap_p_p0, cap_rest_p0, cap_m_p0, cap_r_p0, cap_n_p0, elem_0),
